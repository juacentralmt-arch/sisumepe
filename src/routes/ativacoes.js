...
const { parseAtivacoes, scoreParse, crossValidate } = require('../lib/ativacoesParser');
const { gerarAtivacaoPDF } = require('../lib/ativacoesPdf');
const router = express.Router();

// helper para extrair texto de PDF via pdfjs-dist (on-prem, sem envio externo)
...
// POST /api/ativacoes/auto-preencher  (100% on-prem: regex + validação cruzada, sem LLM)
router.post('/api/ativacoes/auto-preencher', shared.auth(['tecnico','psico','admin']), shared.upload.single('arquivo'), shared.ah(async (req,res)=>{
  ...
  res.json({ ok:true, dados, score, warnings, textoExtraido: text.slice(0,8000), paginas: text.split('\n').length });
}));

// POST /api/ativacoes/parse-text  (texto OCR ou colado) - on-prem
router.post('/api/ativacoes/parse-text', shared.auth(['tecnico','psico','admin']), shared.ah(async (req,res)=>{
  ...
  res.json({ ok:true, dados, score, warnings, textoExtraido: texto.slice(0,8000) });
}));
