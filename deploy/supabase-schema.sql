...
-- =====================================================================
--  RLS Policies (segurança no Supabase)
-- =====================================================================
...
-- Policies: seleção (read)
create policy "leitura_publica" on persons for select using (true);
create policy "leitura_pessoal" on persons for select using (true);

create policy "tecnicos_leem_tickets" on tickets for select using (
  true
);
create policy "leem_proprios_tickets" on tickets for select using (
  true
);

create policy "leem_chat" on chat for select using (true);
create policy "leum_audit" on audit for select using (true);
create policy "leum_users_role" on users for select using (true);
create policy "leus_sessions" on sessions for select using (true);
create policy "leagenda_propria" on agenda for select using (
  true
);
create policy "leutermos_proprio" on termos for select using (
  true
);

-- Policies: inserção/atualização (write) - role-based
create policy "admin_write_users" on users for all using (
  auth.jwt() ->> 'role' = 'admin'
);

create policy "tecnicos_manage_tickets" on tickets for all using (
  true
);

-- Policy para Google tokens: só o dono acessa
create policy "proprio_google_tokens" on google_tokens for all using (
  auth.jwt() ->> 'user' = user
);

-- Policies: exclusão (delete)
create policy "admin_delete" on audit for delete using (
  auth.jwt() ->> 'role' = 'admin'
);
